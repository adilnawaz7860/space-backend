...
function pushSpaceState(spaceId: string) {
  if (!_io) return;
...
function isHostOrCoHost(space: any, userId: string): boolean {
  return (
    space.host.toString() === userId ||
    space.participants.some((p: any) => p.user.toString() === userId && p.role === "cohost")
  );
}

// Helper: update Livekit permissions silently
async function updateLivekitPerms(roomName: string, identity: string, canPublish: boolean) {
  try {
...
spaces.post("/", async (c) => {
  try {
    ...
    if (err instanceof z.ZodError) return c.json({ error: err.errors[0].message }, 400);
...
spaces.post("/:id/join", async (c) => {
  ...
  const space = await Space.findById(c.req.param("id"));
  if (!space) return c.json({ error: "Space not found" }, 404);
  if (space.status !== "live") return c.json({ error: "Space is not live" }, 400);
  const myId = new mongoose.Types.ObjectId(userId);
  ...
  if (space.inviteOnly && !isInvited && !isHost) return c.json({ error: "invite_only" }, 403);
  if (!alreadyIn) {
...
spaces.post("/:id/leave", async (c) => {
  ...
  await Space.findByIdAndUpdate(id, {
    $pull: { participants: { user: new mongoose.Types.ObjectId(userId as string), role: "listener" } }
  });
...
spaces.post("/:id/leave-hard", async (c) => {
  ...
  const id = c.req.param("id");
  await Space.findByIdAndUpdate(id, {
    $pull: { participants: { user: new mongoose.Types.ObjectId(userId as string) } }
  });
  pushSpaceState(id);
  return c.json({ message: "Left space" });
});
...
spaces.post("/:id/end", async (c) => {
  ...
  if (space.host.toString() !== userId) return c.json({ error: "Only host can end" }, 403);
  space.status = "ended";
...
spaces.post("/:id/token", async (c) => {
  ...
  const participant = space.participants.find(
    (p) => p.user.toString() === userId || (p.user as any)._id?.toString() === userId
  );
...
spaces.post("/:id/invite/:targetUserId", async (c) => {
  ...
  if (!space) return c.json({ error: "Not found" }, 404);
  if (!isHostOrCoHost(space, userId)) return c.json({ error: "Only host or co-host can invite" }, 403);
...
spaces.post("/:id/request", async (c) => {
  ...
  const space = await Space.findById(c.req.param("id"));
  if (!space) return c.json({ error: "Not found" }, 404);
  if (!space.inviteOnly) return c.json({ error: "Space is open" }, 400);
  const myId = new mongoose.Types.ObjectId(userId);
  const alreadyRequested = space.joinRequests.some((r) => r.user.toString() === userId);
  if (alreadyRequested) return c.json({ error: "Already requested" }, 409);
...
spaces.post("/:id/request/:requestId/approve", async (c) => {
  ...
  const space = await Space.findById(id);
  if (!space) return c.json({ error: "Not found" }, 404);
  if (space.host.toString() !== userId) return c.json({ error: "Only host can approve" }, 403);
...
spaces.post("/:id/request/:requestId/deny", async (c) => {
  ...
  const space = await Space.findById(id);
  if (!space) return c.json({ error: "Not found" }, 404);
  if (space.host.toString() !== userId) return c.json({ error: "Only host can deny" }, 403);
  const req = space.joinRequests.find((r) => r._id?.toString() === requestId);
...
spaces.post("/:id/speak-request", async (c) => {
  ...
  if (!participant) return c.json({ error: "Not in this space" }, 403);
  if (participant.role !== "listener") return c.json({ error: "Only listeners can request to speak" }, 400);
...
spaces.delete("/:id/speak-request", async (c) => {
  ...
  if (!space) return c.json({ error: "Not found" }, 404);
  const participant = space.participants.find((p) => p.user.toString() === userId);
  if (participant) { participant.handRaised = false; await space.save(); }
  pushSpaceState(spaceId);
...
spaces.patch("/:id/participant/:targetUserId/role", async (c) => {
  ...
  // Permission check
  if (!isHost && !isCoHost) return c.json({ error: "Only host or co-host can assign roles" }, 403);
  // Co-hosts cannot assign co-host role — only host can
  if (!isHost && role === "cohost") return c.json({ error: "Only host can assign co-host role" }, 403);
  // Nobody can change the host's own role
  if (targetUserId === space.host.toString()) return c.json({ error: "Cannot change host role" }, 403);
  if (!["cohost", "speaker", "listener"].includes(role)) return c.json({ error: "Invalid role" }, 400);
  ...
  if (role === "cohost") {
    ...
    if (currentCohosts >= COHOST_LIMIT) return c.json({ error: `Co-host limit reached (max ${COHOST_LIMIT})` }, 400);
  }
  if (role === "speaker") {
    const currentSpeakers = space.participants.filter((p) => p.role === "speaker" && p.user.toString() !== targetUserId).length;
    if (currentSpeakers >= SPEAKER_LIMIT) return c.json({ error: `Speaker limit reached (max ${SPEAKER_LIMIT})` }, 400);
  }
  ...
  if (role === "listener") participant.handRaised = false;
...
spaces.patch("/:id/participant/:targetUserId/mic", async (c) => {
  ...
  if (!isHostOrCoHost(space, userId) && !isSelf) return c.json({ error: "Unauthorized" }, 403);
  // Host/cohost can only MUTE others (not unmute) — speaker controls their own unmute
  if (!isSelf && micOn === true) return c.json({ error: "Cannot unmute others" }, 403);
  const participant = space.participants.find((p) => p.user.toString() === targetUserId);
...
spaces.patch("/:id/hand", async (c) => {
  ...
  if (!space) return c.json({ error: "Not found" }, 404);
  const participant = space.participants.find((p) => p.user.toString() === userId);
  if (participant) participant.handRaised = !participant.handRaised;
  await space.save();
...
spaces.patch("/:id/participant/:targetUserId/hand", async (c) => {
  ...
  const space = await Space.findById(id);
  if (!space) return c.json({ error: "Not found" }, 404);
  if (!isHostOrCoHost(space, userId)) return c.json({ error: "Only host or co-host can lower hands" }, 403);
  const participant = space.participants.find((p) => p.user.toString() === targetUserId);
