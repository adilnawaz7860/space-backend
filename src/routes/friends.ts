...
friends.get("/requests", async (c) => {
  ...
  const user = await User.findById(userId)
    ...
    .lean();
  if (!user) return c.json({ error: "Not found" }, 404);
  const pending = user.friendRequests.filter((r: any) => true); // all requests shown
...
friends.post("/accept/:requesterId", async (c) => {
  ...
  if (!me.friends.some((f) => f.toString() === requesterId)) me.friends.push(reqId);
  if (!requester.friends.some((f) => f.toString() === userId)) requester.friends.push(myId);
...
friends.delete("/:friendId", async (c) => {
  ...
  if (!me || !friend) return c.json({ error: "Not found" }, 404);
